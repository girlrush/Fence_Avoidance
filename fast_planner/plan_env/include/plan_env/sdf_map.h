...
#include <depth_list/DepthList.h>
...
{
  bool check_box_;
  detection_msgs::BoundingBox last_box_;
...
private:
  ...
  typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, geometry_msgs::PoseStamped>
      SyncPolicyImagePose;
  typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image, detection_msgs::BoundingBoxes>
      SyncPolicyImageBoxes;
  typedef shared_ptr<message_filters::Synchronizer<SyncPolicyImagePose>> SynchronizerImagePose;
  typedef shared_ptr<message_filters::Synchronizer<SyncPolicyImageOdom>> SynchronizerImageOdom;
  typedef shared_ptr<message_filters::Synchronizer<SyncPolicyImageBoxes>> SynchronizerImageBoxes;
  ...
  SynchronizerImageBoxes sync_image_boxes_;

  ros::Publisher depth_list_pub_, canny_pub_;
...
// 3차원 좌표를 3차원 Index 로 변환한 뒤, 1차원 배열로 한번 더 변환하여 해당 index 값을 1로 만듬.
...
// 3차원 좌표를 이산화하여 Index 로 변환하고, 3차원 Index 를 1차원으로 변환하여 해당 위치의 값을 반환함
...
// 현재 위치를 이산화하여 index 로 변환함 (Default : 좌표의 소수점 첫째 자리까지만 반영 / resolution_inv_ = 10)
inline void SDFMap::posToIndex(const Eigen::Vector3d& pos, Eigen::Vector3i& id) {

  // map_origin 은 local map 에서 모든 좌표가 최소점이 되는 값임.
  // map_origin 을 더하여 계산함으로써 local map 을 생성할 수 있음. (map_origin 은 음수이다.)
  // 이때, 소수점은 모두 버리므로 resolution_inv 의 값이 클 수록 더욱 세밀한 정보를 저장할 수 있다.
...
// step 만큼 inflate 된 point 들을 pts 에 저장
